...
impl PacketSwitch {
          ...
          if let Some(packet) = lock_on_self.parse_packet_from_a_known_packet_switch(&socket_addr, &buffered_stream) {
            ...
            match lock_on_self.sniffed_packet_metadata.get(&port).cloned() {
              Some(packet_metadata) => {
                debug!("Got a packet! {:?}", packet_metadata);
                lock_on_self.switch_from_stream(&packet_metadata, buffered_stream)?;
              },
              None => error!("Couldn't retrieve packet metadata")
            };
    ...
    loop {
      match action_to_take {
        ...
        Action::FORWARDTORC => {
          ...
          debug!("New action to take: {action_to_take:?}");
        },
      };
    }
    Ok(())
  ...
  fn switch_from_stream(&mut self, packet_metadata: &PacketMetaData, buffered_stream: Vec<u8>) -> Result<(), Error> {
    ...
    debug!("packet message: {:?}, action to take: {:?}", convert_slice_to_string(&buffered_stream), action_to_take);
  ...
  fn forward_packet_to_remote_controller(&mut self, packet_metadata: &PacketMetaData) -> Result<Action, Error> {
    ...
    let mut stream = TcpStream::connect((self.rc_ip_addr, self.rc_port_to_connect_to))?;
    let packet_as_json = serde_json::to_vec(&packet_metadata).expect("Fired!");
    stream.write_all(&packet_as_json).expect("Fired!");

    stream.flush().expect("Fired!");
    stream.shutdown(Shutdown::Write).expect("Fired!");

    let mut response_from_server = Vec::new();
    stream.read_to_end(&mut response_from_server).expect("Fired!");

    let response = serde_json::from_slice::<RemoteControllerResponse>(&response_from_server).expect("Fired!");
